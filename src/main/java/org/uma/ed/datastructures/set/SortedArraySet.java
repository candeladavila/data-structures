...
import org.uma.ed.datastructures.stack.EmptyStackException;
...
public class SortedArraySet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
   ...
   */
  //inicialización del SortedArraySet
  ...
  public static <T> SortedArraySet<T> copyOf(SortedSet<T> that) {
    if (that.isEmpty()){
      throw new UnsupportedOperationException("Esta vacio");
    }
    // Crea un nuevo SortedArraySet usando el mismo comparador que el conjunto 'that'
    SortedArraySet<T> copy = new SortedArraySet<>(that.comparator());

    // Agrega todos los elementos del conjunto 'that' en orden
    for (T element : that) {
      copy.append(element); // usa add o append según lo que corresponda
    }
  ...
  public void insert(T element) {
    ...
    if (!finder.found){ //si elemento no está ya en el SortedArraySet
      ensureCapacity(); //alargamos el array si es necesario
      for (int i = size; i > finder.index; i--){
        elements[i] = elements[i-1];
      }
      elements[finder.index] = element; //ya hemos abierto el hueco para poner meter el elemento
      size ++;
    }
  }
  ...
  public void delete(T element) {
    Finder finder = new Finder(element);
    if (finder.found){
      size --; //decrementamos el tamaño en uno
      for(int i = finder.index; i < size; i++){ //Movemos los elementos a la izquierda para quitar el hueco
        elements[i] = elements[i+1];
      }
      elements[size] = null;
    }
  ...
  public T minimum() {
    ...
    return elements[0]; //como está ordenado entonces el menor está en la posicion 0
  }
  ...
  public T maximum() {
    ...
    return elements[size-1]; //como está ordenado entonces el mayor está en la ultima posicion (size-1)
  }
  ...
  public static <T> SortedArraySet<T> difference(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    SortedArraySet<T> diferencia = new SortedArraySet<>(comparator);
    //tenemos que crear un while para recorrer los dos elementos de los dos SortedSet para calcular la diferencia
    while(element1 != null){ //mientras que queden elementos en el 1 (element1 != null)
      if (element2 == null){ //caso de que el SortedSet 2 == null;
        diferencia.append(element1);
        element1 = nextOrNull(iterator1);
      }
      int cmp = comparator.compare(element1, element2);
      //intersección -> elementos en común
      //diferencia -> elementos que están en el uno pero no en el dos -> elemento
      if (cmp == 0){ // si son iguales -> no se añade
        element1 = nextOrNull(iterator1);
        element2 = nextOrNull(iterator2);
      }
      else if(cmp < 0){
  ...
  public static <T> SortedArraySet<T> difference(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    while (element1 != null) {
      if (element2 == null) {
        ...
        int cmp = comparator.compare(element1, element2);
        if (cmp == 0) {
          // Elementos iguales, no se añade a diferencia y avanzamos ambos iteradores
          element1 = nextOrNull(iterator1);
          element2 = nextOrNull(iterator2);
        } else if (cmp < 0) {
          // Solo elemento1 es único, lo añadimos a diferencia y avanzamos en iterator1
          diferencia.append(element1);
          element1 = nextOrNull(iterator1);
        } else {
          // Elemento de iterator2 es mayor, solo avanzamos iterator2
          element2 = nextOrNull(iterator2);
        }
      }
    }
