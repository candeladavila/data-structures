...
public class SeparateChainingHashTable<K> implements HashTable<K> {
  ...
  //Tenemos esto porque no sabemos que función es la más adecuada para la clave de tipo K, aplicamos el HashCode a la clave y lo pasamos a un valor positivo
  //para después insertarlo en la tabla
  private int hash(K key) {
    return (key.hashCode() & 0x7fffffff) % table.length;
  }

  // current load factor of hash table
  //Calculamos el factor de carga de la tabla
   ...
   */
  //FINDER
    //El uso de la función hash ya está incluido en el Finder (podemos usarlo para hacer el insert y el delete
  private final class Finder {
    int index;
    Node<K> previous, current;

    Finder(K key) {
      index = hash(key); //le aplico el hash
      //si asumimos que esos números nunca están repetidos ya he terminado
      //si al aplicarle la clave ese elemento ya está lo que hacemos es buscarlo y sustituir el contenido

      previous = null; //puede o no estar a null
      current = table[index]; //puede o no estar a null

      //busca y sustituye el contenido
      while ((current != null) && (!current.key.equals(key))) {
        previous = current;
        current = current.next; //mientras que no lo encuentre me desplazo
      }
    }
  }
  ...
  public void insert(K key) {
    ...
    if (finder.current == null) { // si no encontramos el elemento en la tabla
      table[finder.index] = new Node<>(key, table[finder.index]); //machamos el elemento
      size++;
    } else{ // si no lo encontramos ponemos el valor
  ...
  @SuppressWarnings("unchecked")
  //Aumentar la capacidad de la tabla cuando se llena
