...
public class HashBag<T> extends AbstractBag<T> implements Bag<T> {
  ...
  public void delete(T element) {
    Pair<T> index = hashTable.search(Pair.withElement(element));
    if (index != null){ // caso de que el elemento está en la bolsa
      if (index.occurrences() == 1){ // el elemento solo aparece una vez
        hashTable.delete(Pair.withElement(element));
      } else { // el elemento aparece más de una vez
        int oldOccurrences = index.occurrences();
        hashTable.delete(Pair.of(element, oldOccurrences));
        hashTable.insert(Pair.of(element, oldOccurrences - 1)); // lo que hacemos en un insert del elemento con ocurrencia -1
      }
    }
  }
  ...
  @Override
  public void clear() {
    hashTable.clear(); // eliminamos todos los elementos de la bolsa
  }
  ...
  public void insert(T element) {
    Pair<T> index = hashTable.search(Pair.of(element, 1));
    if (index != null){ // en caso de que el elemento ya estuviera aumentamos el número de ocurrencias
      hashTable.insert(Pair.of(element, index.occurrences() + 1));
    } else { // en caso de que el elemento no estuviera lo añadimos con ocurrencia = 1
      hashTable.insert(Pair.of(element, 1));
    }
  }
  ...
  public int occurrences(T element) {
    Pair<T> index = hashTable.search(Pair.withElement(element)); //mirar el caso de que el elemento no esté (para que devuelva 0)
