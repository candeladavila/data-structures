...
public class ArrayList<T> extends AbstractList<T> implements List<T> {
  ...
  public static <T> ArrayList<T> copyOf(ArrayList<T> that) {
    int initialCapacity = Math.max(that.size(), 10); // Use a default capacity of 10 if the list is empty
    ArrayList<T> copy = new ArrayList<>(initialCapacity);
    for (int i = 0; i < that.size(); i++) {
      copy.append(that.get(i));
    }
    return copy;
  }
   ...
   */
  // VALIDATEINDEX permite asegurarnos de que el índice es válido
  // VALIDATEINDEX permite asegurarnos de que el índice es válido
  ...
  public void prepend(T element) {
    ensureCapacity();
    for (int i = size; i > 0; i--){ //desplazamos a la derecha
      elements[i] = elements[i-1];
    }
    elements[0] = element; //definimos el primer elemento del array como nuestro elemento o
    size ++;  //hemos añadido un elemento
  }
  ...
  public void insert(int index, T element) {
    ensureCapacity();
    if(index != size) { //si son iguales entonces simplemente añades al final (append)
      validateIndex(index); //Validamos el índice para comprobar que es un valor válido
      for (int i = size; i > index; i--) { //desplazamos a la derecha
        elements[i] = elements[i - 1]; //asignamos los valores  a la derecha
      }
    }
    elements[index] = element; //definimos el elemento de la posición i como nuestro elemento
    size ++;  //hemos añadido un elemento
  }
  ...
  public void delete(int index) {
    ...
    /*recorremos el array hasta que eliminemos el dato
    desplazamos todo de forma que elements[i + 1] = elements[1] */

    for (int i = index; i < size -1; i++){ //empezamos en el índice que queremos eliminar hasta el final
      elements[i] = elements[i+1];
    }
    size--;
  }
