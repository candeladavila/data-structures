...
public class LinkedList<T> extends AbstractList<T> implements List<T> {
  ...
  public void insert(int index, T element) {
    if(index==size){ //insertion after last element
      ...
      if (index==0){ //insertion at head and list was not empty
        first=new Node<>(element,first);
  ...
  @Override
  public void clear() {
    size = 0; // this will make all nodes unreachable and will be garbage collected
  }
